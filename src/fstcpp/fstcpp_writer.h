...
public:
	...
	inline void SetWriterPackType(WriterPackType pack_type) {
		FST_CHECK(pack_type != WriterPackType::eZlib and pack_type != WriterPackType::eFastLz);
	...
	void EmitValueChange(
		Handle handle, const uint32_t *val, EncodingType encoding = EncodingType::eBinary
	);
	void EmitValueChange(
		Handle handle, const uint64_t *val, EncodingType encoding = EncodingType::eBinary
	);
