...
// User must ensure the pointer is valid for the duration of the StreamPointerReaderHelper
struct StreamPointerReaderHelper {
	const uint8_t* ptr;
	StreamPointerReaderHelper(const uint8_t* ptr_) : ptr(ptr_) {}
...
// User must ensure the pointer is valid for the duration of the StreamPointerWriteHelper
struct StreamPointerWriteHelper {
	uint8_t* ptr;

	StreamPointerWriteHelper(uint8_t* ptr_) : ptr(ptr_) {}

	template<typename T>
	StreamPointerWriteHelper& Write(T u) {
		const size_t s = sizeof(u);
		std::memcpy(ptr, &u, s);
		ptr += s;
		return *this;
	...
	template<typename T>
	StreamPointerWriteHelper& Fill(T u, size_t count) {
		const size_t s = sizeof(u) * count;
		for (size_t i = 0; i < count; ++i) {
			std::memcpy(ptr + i * sizeof(u), &u, sizeof(u));
		}
		return *this;
	}

	template<typename T>
	StreamPointerWriteHelper& Write(T* u, size_t size) {
		const size_t s = sizeof(u) * size;
		std::memcpy(ptr, u, s);
		ptr += s;
		return *this;
	...
	// We do not provide little-endian version since FST only uses big-endian
	template<typename U>
	StreamPointerWriteHelper& WriteUIntBE(U u) {
		u = platform::to_big_endian(u);
		const size_t s = sizeof(u);
		std::memcpy(ptr, &u, s);
		ptr += s;
	...
	// then the first byte will be [9-:8], then {[1:0], 6'b0}.
	template<typename U>
	StreamPointerWriteHelper& WriteUIntPartialForValueChange(U u, size_t bitwidth) {
		// Shift left to align the MSB to the MSB of the uint
		u <<= sizeof(u) * 8 - bitwidth;
		// Write the first (bitwidth+7)/8 bytes
		u = platform::to_big_endian(u);
